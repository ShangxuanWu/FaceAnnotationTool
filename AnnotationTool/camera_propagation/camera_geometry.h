...
__declspec(dllexport) void multiview_optimization_single_point(std::map<std::string, pts_2d_conf>& pts_src, std::vector<mycamera>& camera_cluster, std::map<std::string, pts_2d_conf>& pts_2d, const bool consider_dist = default_consider_dist, const int num_ransac = default_ransac);
...
__declspec(dllexport) void multiview_optimization_multiple_points(std::map<std::string, std::vector<pts_2d_conf>>& pts_src, std::vector<mycamera>& camera_cluster, std::map<std::string, std::vector<pts_2d_conf>>& pts_2d, const bool consider_dist = default_consider_dist, const int num_ransac = default_ransac);
...
__declspec(dllexport) void triangulation_from_two_views(std::vector<pts_2d_conf>& pts_src1, std::vector<pts_2d_conf>& pts_src2, mycamera& camera1, mycamera& camera2, std::vector<pts_3d_conf>& pts_dst, const bool consider_dist = default_consider_dist);
__declspec(dllexport) void triangulation_from_two_views(std::vector<cv::Point2d>& pts_src1, std::vector<cv::Point2d>& pts_src2, mycamera& camera1, mycamera& camera2, std::vector<cv::Point3d>& pts_dst, const bool consider_dist = default_consider_dist);
...
__declspec(dllexport) void multi_view_projection(std::vector<cv::Point3d>& pts_src, std::vector<mycamera>& camera_cluster, std::map<std::string, std::vector<cv::Point2d>>& pts_dst, const bool consider_dist = default_consider_dist);

// support single point
// camera with distortion could also be considered
__declspec(dllexport) void multi_view_projection(cv::Point3d& pts_src, std::vector<mycamera>& camera_cluster, std::map<std::string, cv::Point2d>& pts_dst, const bool consider_dist = default_consider_dist, const bool debug_mode = DEBUG_MODE);
...
// confidence supported	
// camera with distortion could also be considered
__declspec(dllexport) double calculate_projection_error(cv::Point3d& pts_src, std::vector<mycamera>& camera_cluster, std::map<std::string, pts_2d_conf>& pts_2d_given, const bool consider_dist = default_consider_dist);
...
__declspec(dllexport) void get_3d_ray(pts_2d_conf& pts_2d, mycamera& mycamera, cv::Point3d& C, std::vector<double>& ray, const bool consider_dist = default_consider_dist);
