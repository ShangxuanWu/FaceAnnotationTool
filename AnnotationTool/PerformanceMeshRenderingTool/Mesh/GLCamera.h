...
public:
	GLCamera() : lastb(Mouse::NONE), lightdir(vec(0.452749, 0.775398, 0.440201/*0,0,1*/)),
	...
	void setupGL2(const point &scene_center, float scene_size, int scalex, int scaley) const;

	static void GL2CVprojmatrix(float glmat[16], int width, int height,float cvmat[16]);
	static void CV2GLprojmatrix( float cparam[3][4], int width, int height, float gnear, float gfar, float m[16] );
