...
public:
	...
	struct Face {
		...
		Face(const int &v0, const int &v1, const int &v2)
			{ v[0] = v0; v[1] = v1; v[2] = v2; is_obtuse = -1;}
		Face(const int *v_)
			{ v[0] = v_[0]; v[1] = v_[1]; v[2] = v_[2]; is_obtuse = -1;}
	...
	vector<Vec<2,float>> uvcoord;

	vector<float> vetareas;
	...
	{
		this->vertices.resize(pmesh->vertices.size());

		for (int i=0;i<pmesh->vertices.size();i++)
		{
			this->vertices[i][0] = pmesh->vertices[i][0];
			this->vertices[i][1] = pmesh->vertices[i][1];
			this->vertices[i][2] = pmesh->vertices[i][2];
		}

		this->faces.resize(pmesh->faces.size());
		for (int i=0;i<pmesh->faces.size();i++)
		{
			this->faces[i][0] = pmesh->faces[i][0];
			this->faces[i][1] = pmesh->faces[i][1];
			this->faces[i][2] = pmesh->faces[i][2];
		}
	}
