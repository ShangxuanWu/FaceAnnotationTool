...
void GLSLProgram::enableAttribute(const std::string &name)
{
	int loc = getAtrLoc(name);

	glEnableVertexAttribArray(loc);

}

void GLSLProgram::disableAttribute(const std::string &name)
{
	int loc = getAtrLoc(name);
...
int GLSLProgram::getAtrLoc(const std::string &name)
{
	int loc = -1;
	uniformmap::const_iterator it = attributes_.find(name);
	if(it == attributes_.end())
	{
		loc = glGetAttribLocation(handle_, name.c_str());
		if(loc == -1)
		{
			std::string s;
			s.append(name);
		...
		attributes_.insert(uniformmap::value_type(name,loc));
	}
	else
	{
		loc = it->second;
