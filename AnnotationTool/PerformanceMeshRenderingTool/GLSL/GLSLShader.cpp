...
GLSLShader::GLSLShader(const std::string &filename, GLenum shaderType) : compiled_(false)
{
  ...
  GLint length;
  const char * source = readShader(filename,&length);
  glShaderSource(handle_, 1, static_cast<const GLchar**>(&source), &length);
...
char *GLSLShader::readShader(const std::string &filename, GLint *length)
{
