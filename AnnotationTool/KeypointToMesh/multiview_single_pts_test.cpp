...
int main(int argc, char* argv[]) {
	...
	LoadIdCalibration("sequence\\input\\KRT", camera_cluster, false, false);	// read calibration file without skew parameter in the intrinsic
	cout << "reading calibration file successfully" << endl;

	int num_parts = 1;
	int frame = 0;
	double resize_factor = 10;
	...
	for (int i = 0; i < camera_cluster.size(); i++) {
		...
		load_points_with_conf(pose, pts_tmp, 10, 1);	// load only one pts from each file and set resize factor as 10
	...
	std::cout << "It spend " << float(clock() - begin_time) / CLOCKS_PER_SEC << " second to read the mesh file" << std::endl << std::endl;
