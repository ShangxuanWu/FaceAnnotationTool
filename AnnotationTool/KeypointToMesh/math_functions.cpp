...
// Doc: all 2d lines in this file are denoted by using three variables: ax + by + c = 0. There are only two degree of freedom though.
/*********************************************** I/O ******************************************************/
...
std::vector<int> generate_weighted_randomization(std::vector<double>& weights, int size, int seed) {
	...
	ASSERT_WITH_MSG(size >= 1, "The size of random number request should be larger or equal than 1 while generating weighted random number.");
...
std::vector<double> normalize_line_plane(std::vector<double>& src) {
	ASSERT_WITH_MSG(src.size() == 3 || 2, "The size of input line should be vector of 2 or 3 while normalizing.");
